...
file_handler = logging.FileHandler('SearchProcessing.log')
...
class Processing(threading.Thread):
    def __init__(self, twitter_consumer_key, twitter_consumer_secret,
                 ...
                 twitter_geo_param, processor_id, couchdb, since_id=None,
                 max_id=-1, tweets_per_query=100, search_query='*'):
                ...
                if self.max_id <= 0:
                    if not self.since_id:
                        new_tweets = self.api.search(
                            q=self.search_query, count=self.tweets_per_query, geocode=self.twitter_geo_param)
                    else:
                ...
                for twit_obj in new_tweets:
                    try:
                        ...
                        if tweet and tweet['coordinates'] and tweet['coordinates']['coordinates']:
                            ...
                            process_info = {'sentiment': sentiment,
